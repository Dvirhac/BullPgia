...
int main() {
	...
	if (signal == 0) {
		...
		for (uint i=0; i<100; ++i) {
			...
			testcase.CHECK_EQUAL(play(randy, smarty, 4, 100) > 10, false);  // smarty should always win in at most 10 turns!
		}
		...
		for (uint i=0; i<100; ++i) {
			testcase.CHECK_EQUAL(play(randy, smarty, length, 100) <= guesses, true);  // smarty should always win in at most 10 turns!
		}
